...
@Component
public class PokeListRestAdapter implements PokeListRepository {

    private final RestTemplate restTemplate;
    private final RestUtilsProps restUtilsProps;

    public PokeListRestAdapter(RestTemplate restTemplate, RestUtilsProps restUtilsProps) {
        this.restTemplate = restTemplate;
        this.restUtilsProps = restUtilsProps;
        Map<HttpStatus, RuntimeException> map = new HashMap<>();
        map.put(HttpStatus.NOT_FOUND, new NotFoundRestClientException(ErrorCode.POKE_LIST_NOT_FOUND));
        map.put(HttpStatus.INTERNAL_SERVER_ERROR, new ServiceUnavailableException(ErrorCode.POKE_LIST_SERVER_ERROR));
        map.put(HttpStatus.REQUEST_TIMEOUT, new TimeoutRestClientException(ErrorCode.POKE_LIST_TIMEOUT));
    ...
    @Override
    public PokeDomainPaginator findByOffset(int limit, int offset) {
        PokePaginator paginator = Optional.of(
            restTemplate.exchange(
                    this.restUtilsProps.getApiProperties().getPoke().getList(),
                    HttpMethod.GET,
                    this.restUtilsProps.getHeadersAcceptJson(),
                    new ParameterizedTypeReference<PokePaginator>(){},
                    limit, offset))
                .map(HttpEntity::getBody)
                .orElseThrow(()->new NotFoundRestClientException(ErrorCode.POKE_LIST_NOT_FOUND));
            return PokePaginator.fromEntity(paginator);
    }
}
